        ...
        {members.map((member) => {
          ...
          return (
            <section key={member.id} className="flex flex-col gap-2">
              <h2 className="text-sm uppercase tracking-widest text-lime-300/80">
                {member.displayName}
              </h2>
              {theirs.length === 0 && (
                <p className="rounded-2xl border border-white/10 bg-[#1a2118] px-4 py-3 text-sm text-stone-500">
              ...
              {theirs.map((routine) => (
                <Link
                  ...
                  className="rounded-2xl border border-white/10 bg-[#1a2118] px-4 py-3"
                >
